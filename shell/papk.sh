#papk.sh
#---------------------必须修改：渠道数组----------------#
channels=(Mi Tencent Baidu Huawei Oppo S360)
#当前工程绝对路径
project_paths=$(pwd)
...
if (($number == 0 )); then
  echo "=============== 开始构建：全部渠道包 ==============="
  for(( i=0;i<${c_length};i++)) do
    echo "正在构建 All：${channels[$i]} 渠道包"
    flutter build apk -no-sound-null-safety --dart-define=APP_CHANNEL=${channels[$i]}
    cp -R ${release_path}*.apk ${prod_path}
  done;
else
  echo "=============== 正在构建：${channels[$((number-1))]} 渠道包 ==============="
  flutter build apk --no-sound-null-safety --dart-define=APP_CHANNEL=${channels[$((number-1))]}
  cp -R ${release_path}*.apk ${prod_path}
fi
#判断apk目录下是否有文件
